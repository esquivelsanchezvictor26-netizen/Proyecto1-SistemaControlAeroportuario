...
public class ControllerAirport extends Functions {
	...
	public void showBoardingView() {
		...
		v.getBtnAddToQueue().addActionListener(e -> {
			...
			if (!msg.startsWith("ERROR")) {
				filesJson.writeBoardingQueue(reservation.getBoardingQueue(), "queueVuelo" + flightNum + ".json");
			}
		...
		v.getBtnBoardNext().addActionListener(e -> {
			...
			filesJson.writeBoardingQueue(reservation.getBoardingQueue(), "queueVuelo" + flightNum + ".json");
			filesJson.writePassengers(reservation.getBoardedPassengerList(), "boardedVuelo" + flightNum + ".json");
	...
	private void updateBoardingDisplay(BoardingPanel v) {
		...
		// Muestra la cola de pasajeros
		NodeSimpleList<Passenger> currentQueue = reservation.getBoardingQueue().getFirst();
		String queueText = "";
		while (currentQueue != null) {
			queueText += currentQueue.getData().getName() + " (ID: " + currentQueue.getData().getId() + ")\n";
			currentQueue = currentQueue.getNextNode();
		}
		v.getTaQueue().setText(!queueText.isEmpty() ? queueText : "No hay pasajeros en cola.");

		// Muestra los pasajeros abordados
		NodeDoubleList<Passenger> currentBoarded = reservation.getBoardedPassengerList().getHead();
		String boardedText = "";
		while (currentBoarded != null) {
			boardedText += currentBoarded.getData().getName() + " (ID: " + currentBoarded.getData().getId() + ")\n";
			currentBoarded = currentBoarded.getNextNode();
		}
		v.getTaBoarded().setText(!boardedText.isEmpty() ? boardedText : "Ningún pasajero ha abordado.");
	}

	//Metodos Auxiliares de la Controller
	...
	private LogicReservation getOrCreateReservationForFlight(int flightNumber) {
		...
		if (f != null) {
			...
			BoardingQueue loadedQueue = filesJson.readBoardingQueue("queueVuelo" + flightNumber + ".json");
			if (loadedQueue != null) {
				newReservation.setBoardingQueue(loadedQueue);
			}

			DoubleListPassenger loadedBoarded = filesJson.readPassengers("boardedVuelo" + flightNumber + ".json", f.getMaximumCapacity());
	...
	//Metodo para buscar la reservacion correspondiente a un numero de vuel
	private DoubleListPassenger getPassengerListForFlight(int flightNumber) {
	    ...
	    if (res != null) {
	        return res.getPassengerList();//Retorna la lista doble de pasajeros
	    }
	    return null;
	}
}